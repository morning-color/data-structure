...
private:
   int _size; QuadlistNodePosi(T) header; QuadlistNodePosi(T) trailer; //规模、头哨兵、尾哨兵
protected:
...
// 只读访问接口
   ...
   QuadlistNodePosi(T) first() const { return header->succ; } //首节点位置
   QuadlistNodePosi(T) last() const { return trailer->pred; } //末节点位置
   bool valid(QuadlistNodePosi(T) p) //判断位置p是否对外合法
      { return p && (trailer != p) && (header != p); }
// 可写访问接口
   T remove(QuadlistNodePosi(T) p); //删除（合法）位置p处的节点，返回被删除节点的数值
   QuadlistNodePosi(T) //将*e作为p的后继、b的上邻插入
