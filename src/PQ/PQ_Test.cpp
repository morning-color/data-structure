...
void  testHeap(int n) {
   T* A = new T[n/3]; //创建容量为n/3的数组，并
   for (int i = 0; i < n/3; i++) A[i] = dice((T)3*n); //在其中随机生成n/3个词条
   /*DSA*/printf("%d random keys created:\n", n/3);
   /*DSA*/for (int i = 0; i < n/3; i++) print(A[i]); printf("\n");

   H heap(A, n/3); //批量建堆（PQ_ComplHeap实现了Robert Floyd算法）
   delete [] A;
   /*DSA*/print(heap);
